<script>
    ...
    $: activeUsersWithAnimations = activeUsers.map(user => {
    ...
    onMount(() => {
      ...
      const unsubscribeGames = onSnapshot(gamesQuery, (querySnapshot) => {
        ...
        // Start the animation loop after data is loaded
        if (!animationStarted && activeUsersWithAnimations.length > 0) {
          startAnimationLoop();
        }
      });
    ...
    function generateAvatarAnimation(id) {
      ...
      const startX = randomInRange(0, window.innerWidth - 50, seed);
      const startY = randomInRange(0, window.innerHeight - 50, seed + 1);
      ...
      function animate() {
        activeUsersWithAnimations.forEach(user => {
          ...
          if (position.x <= 0) {
            position.x = 0;
            position.vx *= -1;
          } else if (position.x + 50 >= window.innerWidth) {
          ...
          if (position.y <= 0) {
            position.y = 0;
            position.vy *= -1;
          } else if (position.y + 50 >= window.innerHeight) {
          ...
          <div class="ux-dash__game-scores">
            {#each game.topPlayers as player}
              <div class="ux-dash__game-score">
                <Avatar size="sm" src={player.avatarUrl} />
                <div class="ux-dash__game-score-name">{player.email}</div>
